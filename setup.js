...
// ── 1. lib/data.ts ───────────────────────────────────────────────────────────
let d = fs.readFileSync('lib/data.ts', 'utf8');
d = d.split('\n')
      .filter(l => !l.includes('"Blog"') && !l.includes('"Refund Policy"'))
      .join('\n');
d = d.replace('+92 300 1234567', '+1 (647) 616-5995');
d = d.replace('Worldwide \u2014 Remote', 'Toronto, Canada');
d = d.replace('"Privacy Policy", href: "#"',   '"Privacy Policy", href: "/privacy-policy"');
d = d.replace('"Terms of Service", href: "#"', '"Terms of Service", href: "/terms-of-service"');
d = d.replace('"Cookie Policy", href: "#"',    '"Cookie Policy", href: "/cookie-policy"');
fs.writeFileSync('lib/data.ts', d, 'utf8');
console.log('✓ lib/data.ts');

// ── 2. components/footer.tsx ─────────────────────────────────────────────────
let f = fs.readFileSync('components/footer.tsx', 'utf8');
f = f.replace('+1 (234) 567-890', '+1 (647) 616-5995');
f = f.replace('tel:+1234567890', 'tel:+16476165995');
f = f.replace('San Francisco, CA', 'Toronto, Canada');
const t = 'href="#"';
let i = f.indexOf(t);
f = f.slice(0,i) + 'href="/privacy-policy"'   + f.slice(i+t.length);
i = f.indexOf(t);
f = f.slice(0,i) + 'href="/terms-of-service"' + f.slice(i+t.length);
fs.writeFileSync('components/footer.tsx', f, 'utf8');
console.log('✓ components/footer.tsx');

// ── 3. Beber Builds → BeberBuilds ────────────────────────────────────────────
['components/navbar.tsx','components/about.tsx','app/layout.tsx'].forEach(p => {
  fs.writeFileSync(p, fs.readFileSync(p,'utf8').replace(/Beber Builds/g,'BeberBuilds'), 'utf8');
  console.log('✓ ' + p);
});

// ── 4. components/cta-banner.tsx ─────────────────────────────────────────────
fs.writeFileSync('components/cta-banner.tsx', `"use client";
...
`, 'utf8');
console.log('✓ components/cta-banner.tsx');

// ── 5. Legal pages ────────────────────────────────────────────────────────────
fs.mkdirSync('app/privacy-policy',  { recursive: true });
fs.mkdirSync('app/terms-of-service',{ recursive: true });
fs.mkdirSync('app/cookie-policy',   { recursive: true });

fs.writeFileSync('app/privacy-policy/page.tsx', `import Link from "next/link";
...
`, 'utf8');
console.log('✓ app/privacy-policy/page.tsx');

fs.writeFileSync('app/terms-of-service/page.tsx', `import Link from "next/link";
...
`, 'utf8');
console.log('✓ app/terms-of-service/page.tsx');

fs.writeFileSync('app/cookie-policy/page.tsx', `import Link from "next/link";
...
`, 'utf8');
console.log('✓ app/cookie-policy/page.tsx');

console.log('\n✅ Done! Now run:\n  git add -A\n  git commit -m "feat: UI updates, legal pages, BeberBuilds branding"\n  git push origin master');